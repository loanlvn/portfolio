...
const SOCIALS = [
  { name: "LinkedIn", href: "https://www.linkedin.com/in/loan-lavigne-813086356/", Icon: LinkedInIcon },
...
export default function Landing() {
  ...
  const progress = useMemo(() => ((10 - seconds) / 10) * 100, [seconds]);

  useEffect(() => {
    if (paused) return;
    if (seconds <= 0) { navigate("/portfolio"); return; }
    ...
    <PageTransition>
      <div className="grid place-items-center min-h-[100svh]">
        <section className="container max-w-xl text-center">
          <h1 className="text-4xl md:text-6xl font-bold tracking-tight">Loan Samuel Lavigne</h1>
          ...
          <div className="mt-10 flex flex-col items-center gap-3">
            <div className="text-sm text-muted-foreground">
              Redirection in <span className="font-semibold text-foreground">{seconds}s</span>
            </div>
